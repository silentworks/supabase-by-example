...
router.get("/signin/otp", function (req, res, next) {
  res.render("auth/magiclink", { email: "", title: "Sign In" });
});
router.post("/signin/otp", async function (req, res, next) {
  ...
  try {
    ...
    if (err instanceof ZodError) {
      const errs = formatError(err);
      req.flash("errors", errs);
      return res.render("auth/magiclink", { errors: errs, email });
    }
  }
  ...
  if (error) {
    ...
    return res.render("auth/magiclink", fault(error.message, { email }));
  }
